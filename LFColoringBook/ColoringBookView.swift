...
//A "frozen context" to actually draw, based on Apple's https://developer.apple.com/documentation/uikit/touches_presses_and_gestures/illustrating_the_force_altitude_and_azimuth_properties_of_touch_input
    ...
    public func hideColoringLayer() {
        guard let cgImage = frozenContext.makeImage() else { return }
        let   bottomImage = UIImage(cgImage: cgImage, scale: 1.0, orientation: .downMirrored)
 
        var topImage = UIImage(cgImage: self.coloringImage.cgImage)

        var size = CGSize(width: self.pixelSize.width, height: self.pixelSize.height)
...
//
//        image.maskWithColor(color: UIColor.black)
//        print( image.maskWithColor(color: UIColor.blue))
//
//        UIImageWriteToSavedPhotosAlbum(image,nil,nil,nil)

        }
//    public func hideColoringLayer(){
//        print(imageFromLayer(layer: self.coloringLayer))
//    UIImageWriteToSavedPhotosAlbum(imageFromLayer(layer: self.coloringLayer),nil,nil,nil)
...
//    }
    ...
    private func redrawInFrozenContexts(updateScreen:Bool = true){
